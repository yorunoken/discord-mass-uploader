import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ChatInputCommandInteraction, ComponentBuilder, EmbedBuilder, SlashCommandBuilder, TextChannel } from "discord.js";
import fs from "fs";
...
export async function run({ interaction }: { interaction: ChatInputCommandInteraction }) {
  ...
  if (threadExists) {
    return interaction.editReply(`The thread with name ${fileName}.${fileFormat} already exists. Please try changing the name of the file.`);
  }
  ...
  for (let i = 0; i < totalChunks; i++) {
    ...
    await interaction.editReply(`Uploaded chunk ${i + 1} of ${totalChunks} (${(((i + 1) / totalChunks) * 100).toFixed(2)}%)`);
  }

  await interaction.editReply({
    content: `The file "${fileName}.${fileFormat}" has been uploaded. You can download the file to your computer by clicking on the green "Download" button.`,
    embeds: [new EmbedBuilder().setTitle(`File information`).setDescription(`Name: ${fileName}\nType: ${fileFormat}\nSize: ${(fileData.length / 1024 / 1024).toFixed(2)}Mb\nThread ID: ${thread.id}`)],
    components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("download").setStyle(ButtonStyle.Success).setLabel("Download")) as any],
  });
}
